...
function loadShifts() {
  try {
    const raw = localStorage.getItem(LS_KEY_SHIFTS);
...
// Загружаем сохранённые данные или генерируем начальные
let MOCK_SHIFTS = loadShifts() || (() => {
  const defaults = DEFAULT_SHIFTS();
  saveShifts(defaults);
  return defaults;
})();
...
export const api = {
  ...
  async createShift(data) {
    await delay(200);
    const newShift = { ...data, id: nextId() };
    MOCK_SHIFTS.push(newShift);
    saveShifts(MOCK_SHIFTS);
